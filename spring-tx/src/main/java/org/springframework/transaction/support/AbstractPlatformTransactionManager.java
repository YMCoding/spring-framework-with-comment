...
public abstract class AbstractPlatformTransactionManager implements PlatformTransactionManager, Serializable {
	...
	public final TransactionStatus getTransaction(@Nullable TransactionDefinition definition) throws TransactionException {
		...
		// 判断当前线程是否存在事务
		// 判断依据为当前线程记录的连接不为空且连接中的transactionActive属性不为空
		...
		if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_MANDATORY) {
			...
			try {
				...
				//完善transaction 包括设置ConnectionHolder 隔离级别 timeout . 如果是新连接，绑定到当前线程。
	...
	private TransactionStatus handleExistingTransaction(
		...
		// 标示当前方法必须在它自己的事务里运行，一个新的事务将被启动，
		// 而如果有一个事务正在运行的话，则在这个方法运行期被挂起
		if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_REQUIRES_NEW) {
			if (debugEnabled) {
				logger.debug("Suspending current transaction, creating new transaction with name [" +
						definition.getName() + "]");
			}
			// 新事务的建立（使用suspend方法将原事务挂起，挂起的目的是为了在当前执行完毕后将原事务还原
		...
		// 嵌入式事务的处理
		// 表示如果当前正有一个事务在运行中。则该方法应该运行在一个嵌套的事务中
		...
		if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_NESTED) {
			...
			if (useSavepointForNestedTransaction()) {
				...
				// 如果没有可以使用保存点的方式控制事务回滚，那在嵌入式事务的建立初始 简历保存点
	 ...
	 */
	@Nullable
	// 对于挂起操作的目的是记录原有事物的状态，以便于后续操作对于事务的恢复
	...
	private void processRollback(DefaultTransactionStatus status, boolean unexpected) {
		try {
				...
				if (status.hasSavepoint()) {
					...
					if (status.hasTransaction()) {
						if (status.isLocalRollbackOnly() || isGlobalRollbackOnParticipationFailure()) {
							...
							// 当前事务不是独立的事务，那么只能标记状态，等到事务链执行完毕后统一回滚
			...
			// 激活所有TransactionSychronization中对应的方法
			triggerAfterCompletion(status, TransactionSynchronization.STATUS_ROLLED_BACK);
